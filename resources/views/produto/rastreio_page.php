...
function fncRastreiaLote($Lote = null)
{
    ...
    try {
        if ($Lote) {
            ...
            $sql = "SELECT TL.*,
                    TProd.descricao as descricao_prod,
                    TProd.ncm,
                    TProd.GTIN,
                    TProd.Tipo,
                    TP.id as p_id, 
                    TP.razao_social as p_razao_social, 
                    TP.cnpj as p_cnpj, 
                    TP.fone as p_fone, 
                    TP.endereco as p_endereco, 
                    TP.numero as p_numero, 
                    TP.compl as p_compl, 
                    TP.bairro as p_bairro, 
                    TP.cidade as p_cidade, 
                    TP.uf as p_uf, 
                    TP.CEP as p_CEP, 
                    TP.inscr_estadual as p_inscr_estadual, 
                    
                    TP2.id as d_id, 
                    TP2.razao_social as d_razao_social, 
                    TP2.cnpj as d_cnpj, 
                    TP2.fone as d_fone, 
                    TP2.endereco as d_endereco, 
                    TP2.numero as d_numero, 
                    TP2.compl as d_compl, 
                    TP2.bairro as d_bairro, 
                    TP2.cidade as d_cidade, 
                    TP2.uf as d_uf, 
                    TP2.CEP as d_CEP, 
                    TP2.inscr_estadual as d_inscr_estadual 
                    
                    FROM tb_lote AS TL 
                    INNER JOIN tb_produtos AS TProd ON TProd.id = TL.id_produto 
                    INNER JOIN tb_parceiros AS TP ON TP.id = TL.origem_declarada 
                    INNER JOIN tb_parceiros AS TP2 ON TP2.id = TL.distribuidor 
            WHERE rastreio = '" . $Lote . "'";
            ...
            $result = mysqli_query($GLOBALS['conexao2'], $sql);
            $row = mysqli_num_rows($result);
...
//echo $parseQueryString['rastreio'];


$lote = fncRastreiaLote($parseQueryString['rastreio']);
