...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["descricao"]))){
        ...
        $sql = "SELECT id FROM tb_produtos WHERE descricao = '{$descricao}'";
        ...
        if($row >= 1){
            $descricao_err = "Este produto já foi registrado.";
            exit;
        } else{
    ...
    if(empty($descricao_err)){ // && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO tb_produtos (descricao, NCM, GTIN, tipo, unidade) VALUES ('{$descricao}', '{$NCM}', '{$GTIN}', '{$tipo}', '{$unidade}' )";
        //echo $sql;
