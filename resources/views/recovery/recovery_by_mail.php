
<!DOCTYPE html>
<meta http-equiv="Content-Type" content="text/html;charset=utf-8" />
<html>



<header>

    <? 

        require_once $_SERVER['DOCUMENT_ROOT'].'/conexao.php';
        include_once $_SERVER['DOCUMENT_ROOT'].'/menu.php';

        setlocale(LC_ALL,'pt_BR.UTF8');
        mb_internal_encoding('UTF8'); 
        mb_regex_encoding('UTF8');
        header('Content-Type: text/html; charset=UTF-8');
        $code = $_GET['code'];
 
        // Processing form data when form is submitted
       
            if($code){ // && empty($password_err) && empty($confirm_password_err)){
                
                // Prepare an insert statement
                //$sql = "INSERT INTO users (username, password) VALUES (?, ?)";
        
                $sql = "SELECT email FROM tb_clientes WHERE recuperar = '" . $code . "'";
                //echo $sql;

                if ($conexao->query($sql) === TRUE) {
                    //echo "New record created successfully";
                    //header ('Location: list_prod_page.php');
                } else {
                    //echo "Error: " . $sql . "<br>" . $conn->error;
                }

                $result = mysqli_query($conexao, $sql);

                $row = mysqli_num_rows($result);

                if ($row > 0){
                    
                }
                else{
                    header ('Location: list_prod_page.php');
                    exit();
                }

                //echo $result;

            }
    ?>

    <meta charSet="utf-8" />
    <meta name="viewport" content="width=device-width,initial-scale=1.0" />
    <meta name="viewport" content="width=device-width,minimum-scale=1,initial-scale=1" class="next-head" />
    <meta charSet="utf-8" class="next-head" />
    <title>Frutas Dourado - Recuperar senha</title>
<!-- <link rel="preload" href="../register/register.js" as="script" /> -->
    <!-- <link rel="preload" href="https://assets-cdn.inmagine.com/_next/static/wWs51jrH-QY8zL4qFKeDf/pages/_app.js" as="script" />
    <link rel="preload" href="https://assets-cdn.inmagine.com/_next/static/runtime/webpack-f5e50b6b501ccea2a79b.js" as="script" />
    <link rel="preload" href="https://assets-cdn.inmagine.com/_next/static/chunks/commons.17aa3c0be8717670f8e7.js" as="script" />
    <link rel="preload" href="https://assets-cdn.inmagine.com/_next/static/chunks/styles.b7764b24982e27378b63.js" as="script" />
    <link rel="preload" href="https://assets-cdn.inmagine.com/_next/static/runtime/main-1b7293a34bcc89c881b5.js" as="script" />-->
    <link rel="stylesheet" href="../register/commons.css" />
    <link rel="stylesheet" href="../register/styles.css" /> 
</head>

<body>
    <div id="__next"><span class="x-text-regular" lang="US">
            <section class="container-fluid root-container">
                <div class="row mx-0">
                    <div class="layout">
                        <div class="mx-auto position-absolute" style="left:0;right:0;z-index:1;max-width:500px;height:0px;top:-0px"></div>
                        <div class="layout-container">

                            <div class="col-12 col-md-12 col-xl-6 m-0 layout-left-container" style="background-image: url('../images/bg_2.jpg') ;">
                                <div class="x-row layout-logo-row">
                                    <div class="col-12 text-center position-relative layout-logo-container" style="margin-bottom:3.125rem"><img src="../images/trator3.png" class="layout-logo" /></div>
                                </div>
                                <div class="x-row p-0" style="margin-bottom:0.625rem">
                                    <div class="col-12 text-center"><span class="x-text-header" style="color:white;font-size:1.5625rem;line-height:1.5rem;font-weight:lighter">RASTREIO SIMPLES, RÁPIDO, FÁCIL</span></div>
                                </div>
                                <div class="x-row p-0" style="margin-bottom:4.125rem">
                                    <div class="col-12 text-center px-0"><span class="x-text-subtitle" style="color:white;font-size:1rem;line-height:1.125rem;font-weight:lighter;text-transform:none">Faça login para cadastrar seus produtos e seus lotes</span></div>
                                </div>
                                <div class="x-row p-0" style="margin-bottom:1.875rem">
                                    <div class="col-4 px-0"><img src="../images/qrcode.png" style="height:100px; width:100px;" /></div>
                                    <div class="col-8 pr-0">
                                        <div class="x-row"><span class="x-text-subtitle" style="color:white;font-size:1rem;line-height:1.3125rem;text-transform:none;font-weight:normal">Códigos de rastreio QR-Code</span></div>
                                        <div class="x-row"><span class="x-text-subtitle" style="color:white;font-size:1rem;line-height:1.3125rem;text-transform:none;font-weight:lighter">Rastreie e disponibilize para seu cliente o rastreio do produto</span></div>
                                    </div>
                                </div>
                                <div class="x-row p-0">
                                    <div class="col-4 px-0"> <!--<img src="https://assets-cdn.inmagine.com/static/dsai.svg" /> --></div>
                                    <div class="col-8 pr-0">
                                        <div class="x-row"><span class="x-text-subtitle" style="color:white;font-size:1rem;line-height:1.3125rem;text-transform:none;font-weight:normal">Com poucos passos será possível mostrar para o cliente até mesmo o local de produção e plantio do produto</span></div>
                                        <div class="x-row"><span class="x-text-subtitle" style="color:white;font-size:1rem;line-height:1.3125rem;text-transform:none;font-weight:lighter">Obtenha a versão pró sem propagandas</span></div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-10 col-sm-10 col-md-10 col-xl-6 layout-content">
                                <div class="layout-image-top-text-container">
                                    <div class="x-row layout-logo-row">
                                        <div class="col-12 text-center position-relative layout-logo-container"><img src="https://assets-cdn.inmagine.com/static/inmagine-logo.svg" class="layout-logo" /></div>
                                    </div>
                                    <div class="x-row layout-image-top-text-hide-mobile-row">
                                        <div class="col-12 text-center"><span class="x-text-header layout-image-top-text layout-image-text-header">RASTREIO SIMPLES, RÁPIDO, FÁCIL</span></div>
                                    </div>
                                    <div class="x-row layout-image-top-text-subtitle-row">
                                        <div class="col-12 text-center px-0"><span class="x-text-subtitle layout-image-top-text layout-image-top-text-subtitle">Com poucos passos será possível mostrar para o cliente até mesmo o local de produção / plantio do produto</span></div>
                                    </div>
                                </div>

                                <div class="container layout-right-container">
                                    <div class="x-row">
                                        <div class="col-12 container text-center layout-text-title-container">
                                            <div class="x-row layout-text-title-row"><span class="x-text-subtitle mx-auto layout-text-title xbold">Recuperação de senha</span></div>
                                            <div class="x-row layout-text-platform-title-row"><span class="x-text-subtitle mx-auto layout-text-platform-title">Frutas Dourado</span></div>
                                        </div>
                                    </div>
                                    <div class="x-row layout-content-form">
                                        <div class="col-12 px-0">
                                            <form action="../login/mudar_senha.php" method="POST">
                                                <div class="x-row">
                                                    <div class="col-12 px-0"><span class="x-text-label text-capitalize layout-text-label" for="usuario">Email</span>
                                                        <div class="form-group layout-form-group form-group"><input type="text" id="usuario" name="usuario" value="" placeholder="john.doe@gmail.com" class="form-control layout-form-input text-lowercase inputStandard--rVbLh form-control" /></div>
                                                    </div>
                                                </div>
                                                <div class="x-row layout-mb-20 layout-mb-md-12">
                                                    <div class="col-12 px-0"><span class="x-text-label text-capitalize layout-text-label" for="senha">Nova senha</span>
                                                        <div class="position-relative" tabindex="1">
                                                            <div class="form-group layout-form-group form-group"><input type="password" id="senha" name="senha" value="" placeholder="****************" class="form-control layout-form-input inputStandard--rVbLh form-control" /></div><img class="position-absolute" src="https://assets-cdn.inmagine.com/static/icons/eye_slash.svg" style="height:1.5rem;width:1.5rem;top:50%;right:0;cursor:pointer;transform:translate(-0.625rem, -50%)" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="x-row layout-mb-20 layout-mb-md-12">
                                                    <div class="col-12 px-0"><span class="x-text-label text-capitalize layout-text-label" for="senha">Confirme nova senha</span>
                                                        <div class="position-relative" tabindex="1">
                                                            <div class="form-group layout-form-group form-group"><input type="password" id="senha2" name="senha2" value="" placeholder="****************" class="form-control layout-form-input inputStandard--rVbLh form-control" /></div><img class="position-absolute" src="https://assets-cdn.inmagine.com/static/icons/eye_slash.svg" style="height:1.5rem;width:1.5rem;top:50%;right:0;cursor:pointer;transform:translate(-0.625rem, -50%)" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="x-row">
                                                    <div class="col-6 px-0 d-flex">
                                                        <div class="m-0 form-group">
                                                            <!-- <div class="checkbox--7su89 custom-checkbox custom-control"><input type="checkbox" id="checkbox2" name="checkbox2" value="checkbox2" class="custom-control-input" /><label class="custom-control-label" for="checkbox2">Lembrar </label></div> -->
                                                        </div>
                                                    </div>
                                                    <div class="col-6 px-0 d-flex"><a class="d-inline-block align-self-center font-weight-bolder nav-link text-nowrap py-0 ml-auto p-0 layout-text" style="color:#ee5e25" id="forgot-password-link" href="../register/recovery_page.php">Esqueci a senha</a></div>
                                                </div>
                                                <div class="x-row layout-btn-row">
                                                    <div class="col-12 px-0 text-center"><button type="submit" id="login-btn" class="btn primary text-nowrap layout-fullsizable-btn btn btn-primary"><span class="x-text-button d-block layout-text-btn light">Mudar senha</span></button></div>
                                                </div>
                                                <div class="x-row layout-last-row">
                                                    <div class="col-12 px-0 text-center">
                                                        <p class="text-center mb-0 align-center container p-0"><span class="x-text-regular xbold layout-text">Não possui cadastro ainda?</span><a class="d-inline-block align-self-center font-weight-bolder nav-link text-nowrap py-0 px-8 layout-text" style="color:#ee5e25;text-transform:none" id="sign-up-here-btn" href="../register/register_page.php">Registre-se agora</a></p>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                                <!-- <div class="x-row layout-image-top-text-platform-row">
                                    <div class="col-6 layout-ecosystem-logo"><img src="https://assets-cdn.inmagine.com/static/pixlr.svg" /></div>
                                    <div class="col-6 layout-ecosystem-logo"><img src="https://assets-cdn.inmagine.com/static/dsai.svg" /></div>
                                </div> -->
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </span>

</html>